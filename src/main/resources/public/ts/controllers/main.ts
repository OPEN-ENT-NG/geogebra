...
export const mainController = ng.controller('MainController', ['$timeout','$scope', 'route', ($timeout, $scope, route) => {
    ...
    $scope.init = () : void => {
        ...
        const int = setInterval(async function () {
            ...
            if (app && app.exists) {
                ...
                if(hash.length > 0){
                    window.documentId = hash.substring(hash.indexOf("#/") + 2, hash.indexOf("?fileName="));
                    window.fileName = hash.substring(hash.indexOf("?fileName=") + 10, hash.indexOf(".ggb") + 4)
                }
    ...
    const setGGBAplet = async (id: string) : Promise<void> => {
        ...
        await Utils.safeApply($scope);
        navigationGuardService.reset("geogebra");
        window.location.hash = `${id}?fileName=${fileName}`;
    ...
    $scope.saveDocument = (name: string) : void => {
        try {
            ...
            let file : File = new File([u8arr], name + ".ggb", {type: 'application/octet-stream'});
    ...
    function extractNameAndIdProject() : { fileName: string, id: string } {
        ...
        if (!fileName.endsWith(".ggb")) fileName += ".ggb";
    ...
    $scope.updateGGBFile = async () : Promise<void> => {
        try {
            const u8arr : Uint8Array = extractedFileBinary();
            const {fileName, id} = extractNameAndIdProject();
            let file : File = new File([u8arr], fileName, {type: 'application/octet-stream'});
            let doc : Document = new Document();
            doc.name = fileName;
            doc._id = id;
